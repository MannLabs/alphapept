...
__all__ = ['load_thermo_raw', 'load_bruker_raw', 'one_over_k0_to_CCS', 'extract_mzml_info', 'load_mzml_data',
           'extract_nested', 'extract_mq_settings', 'parse_mq_seq', 'get_peaks', 'get_centroid', 'gaussian_estimator',
           'centroid_data', 'get_most_abundant', 'list_to_numpy_f32', 'HDF_File', 'MS_Data_File', 'raw_conversion']
...
def load_mzml_data(filename, most_abundant, callback=None, **kwargs):
    ...
    logging.info('Start reading mzML file...')
    for idx, i in enumerate(spec_indices):
        try:
            spec = next(reader)
            scan_list.append(i)
            ...
            logging.info(f"Bad scan={i} in raw file '{filename}'")
...
def _read_DDA_query_data(
    ...
    if ext.lower() == '.raw':
        ...
        vendor = "mzml file"
        logging.info(f'File {base} has extension {ext} - converting from {vendor}.')
        query_data, acquisition_date_time = load_mzm(
            file_name,
            most_abundant,
            callback=callback,
        )
