    ...
    if not os.path.isdir(file_folder):
        ...
        with st.spinner("Parsing folder"):
            ...
            if len(raw_files) == 0:
                ...
                file_df["Sample"] = [str(i+1) for i in range(len(file_df))]
                ...
                with st.expander("Additional info"):
                    st.write(
                        ...
                        " \n- Sample: For fractionated samples. The same sample will be scored together. If dataset is not fractionated leave as is."
                ...
                recorder["experiment"]["sample"] = file_df_selected[
                    "Sample"
                ].values.tolist()
                recorder["experiment"]["matching_groups"] = matching_groups

                f_dict = file_df_selected.groupby('Sample')['Filename'].unique().to_dict()
