...
__all__ = ['calculate_sequence_coverage', 'get_latest_version']
...
def get_latest_version()->(str):
    """
    Check latest alphapept version from github MannLabs/alphapept/master/settings.ini
    return:
        str: latest alphapept version number
    """

    version_url = 'https://raw.githubusercontent.com/MannLabs/alphapept/master/settings.ini'

    try:
        import urllib.request
        with urllib.request.urlopen(version_url, timeout = 5) as response:
            data = response.read().decode().split('\n')
            latest_version_line = [_ for _ in data if 'version = ' in _]
            latest_version = latest_version_line[0].split('=')[1].strip()
    except:
        return ''
