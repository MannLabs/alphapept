...
__all__ = ['raw_to_centroid', 'connect_centroids_unidirection', 'convert_connections_to_array',
           'eliminate_overarching_vertex', 'path_finder', 'find_path_start', 'find_path_length', 'fill_path_matrix',
           'convert_to_coord_path', 'find_centroid_connections', 'get_hills', 'connect_centroids', 'get_hills_',
           'smooth_mean', 'smooth_median', 'smooth', 'get_minima', 'split_hills', 'split_hill', 'split_recursive',
           'filter_hills', 'weighted_average', 'get_hill_stats', 'get_hill_data_numba', 'get_hill_data',
           'check_isotope_pattern', 'DELTA_M', 'DELTA_S', 'cosinec', 'extract_edge', 'get_edges', 'plot_pattern',
           'check_isotope_pattern_directed', 'grow', 'grow_trail', 'get_trails', 'isolate_isotope_pattern',
           'check_averagine', 'pattern_to_mz', 'cosine_averagine', 'int_list_to_array', 'mz_to_mass', 'get_minpos',
           'get_local_minima', 'is_local_minima', 'truncate', 'M_PROTON', 'get_isotope_patterns',
...
def get_hills(centroids, from_idx, to_idx, min_hill_length=3):
    ...
    hill_ptrs = cupy.empty((path_starts.shape[0] + 1), dtype=cupy.int)

    hill_ptrs[0] = 0
    hill_ptrs[1:] = path_node_cnt.cumsum()
    hill_data = cupy.empty((int(hill_ptrs[-1])), np.int)

    fill_path_matrix(path_starts, forward, hill_data, hill_ptrs)

    del from_idx, to_idx, path_starts, path_node_cnt, forward, backward
...
def get_hills_(query_data, max_gap=2, ppm_tol=8):
    ...
    from_idx, to_idx = connect_centroids(rowwise_peaks, row_borders, centroids, max_gap=max_gap, ppm_tol=ppm_tol)
    hill_ptrs, hill_data = get_hills(centroids, from_idx, to_idx)

    completed_hills__ = []

    if cupy.__name__ == 'numpy':
        indices_  = indices
        hill_ptrs_ = hill_ptrs
        hill_data_ = hill_data
    else:
        indices_ = indices.get()
        hill_ptrs_ = hill_ptrs.get()
        hill_data_ = hill_data.get()

    idx_ = np.searchsorted(indices_, hill_data_, side='right') - 1

    for i in range(len(hill_ptrs) - 1):
        start = hill_ptrs_[i]
        end = hill_ptrs_[i + 1]

        row_ = hill_data_[start:end]
        to_input = idx_[start:end]

        row_idx = to_input
        col_idx = row_ - indices_[to_input]

        completed_hills__.append([(a,b) for a,b in zip(row_idx, col_idx)])

    return completed_hills__
...
def get_hill_stats(hill_data, hill_nboot = 300, hill_nboot_max = 300):
    ...
    return average_mz, delta_m, min_rt, max_rt, int_sum


@njit
def get_hill_data_numba(hill_data):
    hill_stats = np.zeros((len(hill_data), 5))
    for i in range(len(hill_data)):
        average_mz, delta_m, min_rt, max_rt, int_sum = get_hill_stats(hill_data[i])
        hill_stats[i, 0] = average_mz
        hill_stats[i, 1] = delta_m
        hill_stats[i, 2] = min_rt
        hill_stats[i, 3] = max_rt
        hill_stats[i, 4] = int_sum
...
def feature_finder_report(isotope_patterns, isotope_charges, sorted_stats, sorted_data, sorted_hills, query_data, callback=None):
    ...
    for runner in range(len(isotope_patterns)):
        ...
        max_pattern = np.argmax([max(sorted_data[_]['int']) for _ in isotope_patterns[runner]])

        mz = np.min(isotope_data['mz'])

        mz_std = np.mean(sorted_stats[isotope_patterns[runner]]['delta_m'])
        ...
        rt_min_ = isotope_data['rt'].min()
        rt_max_ = isotope_data['rt'].max()

        rt_range = np.linspace(rt_min_, rt_max_, 100)
        trace_sum = np.zeros_like(rt_range)


        for _ in isotope_patterns[runner]:

            rts = sorted_data[_]['rt']
            ints =  sorted_data[_]['int']

            interpolation = np.interp(rt_range, rts, ints)
        ...
        int_sum = np.trapz(trace_sum, rt_range)
        ...
        if not skip:
            ...
            if not settings['general']["find_features"]:
                ...
                if datatype == 'thermo':
                    ...
                    completed_hills = get_hills_(query_data)
